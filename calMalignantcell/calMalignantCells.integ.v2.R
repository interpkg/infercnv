...
if (rm_ct != 'na'){
    ...
    metadata_filtered <- metadata_filtered %>% filter(!(cell_type2 %in% remove_cell_type2))
}

print(unique(metadata_filtered$cell_type2))
...
for (s in unique(df_cnv_gene$sample)){
    ...
    ds_report <- metadata_filtered[metadata_filtered$Sample==s, c('Sample', 'seurat_clusters', 'cell_type', 'cell_type2')]
...
p <- MalignantCellPercentageForCellType(df_merged)
ggsave(paste0(outdir, '/malignantCells.celltype.pdf'), width = 4, height = 2.5)
...
# based on non-immune cells
df_merged$cell_group <- 'Non-immune'
df_merged$cell_group[df_merged$cell_type2 %in% c('Immune', 'Microglia')] <- 'Immune'

df_merged_nonImmune <- df_merged[df_merged$cell_group == 'Non-immune',]
# output non-immune
write.table(df_merged_nonImmune, paste0(outdir, '/infercnv.estimate_malignant_cells.Non-Immune', cutoff_region_size, 'Mb.xls'), sep='\t', quote=FALSE, row.names=FALSE)


# plot Non-immune
# malignant cell percentage per sample
p <- MalignantCellPercentagePerSample(df_merged_nonImmune)
sample_size <- length(unique(df_merged$Sample))
width <- 3.5
if (sample_size > 10){ width <- 6 }
ggsave(paste0(outdir, '/malignantCells.sample.Non-Immune.pdf'), width = width, height = 3)


# malignant cell percentage per cell type2
p <- MalignantCellPercentageForCellType(df_merged_nonImmune)
ggsave(paste0(outdir, '/malignantCells.celltype.Non-Immune.pdf'), width = 4, height = 2.5)
